...
UPDATE public.physio_profiles
SET profile_state = 'fresh'
WHERE profile_state IS NULL;

ALTER TABLE public.physio_profiles
  ALTER COLUMN profile_state SET DEFAULT 'fresh';

ALTER TABLE public.physio_profiles
  ALTER COLUMN profile_state SET NOT NULL;

ALTER TABLE public.physio_profiles
  DROP CONSTRAINT IF EXISTS physio_profiles_profile_state_check;

ALTER TABLE public.physio_profiles
  ADD CONSTRAINT physio_profiles_profile_state_check
  CHECK (profile_state IN ('fresh', 'semi_fatigued', 'fatigued'));
...
SELECT
  ...
  (
    ...
    WHERE pp.athlete_id = lh.athlete_id
      AND pp.profile_state = 'fresh'
      AND pp.valid_from <= (lh.date + interval '1 day')
    ORDER BY pp.valid_from DESC
    LIMIT 1
  ) AS poids,
  CASE
    WHEN swc.current_points >= 4 THEN ROUND(swc.ln_rmssd_7d_avg::numeric, 3)
    ELSE NULL::numeric
  END AS ln_rmssd_7d_avg,
  CASE
    WHEN swc.baseline_points >= 7 THEN ROUND(swc.swc_mean_28d::numeric, 3)
    ELSE NULL::numeric
  END AS swc_mean_28d,
  CASE
...
CREATE OR REPLACE VIEW public.view_athlete_profiles_active AS
SELECT
  ath.first_name || ' ' || ath.last_name AS athlete,
  pp.sport,
  pp.lt1_hr,
...
FROM public.physio_profiles pp
JOIN public.athletes ath ON pp.athlete_id = ath.id
