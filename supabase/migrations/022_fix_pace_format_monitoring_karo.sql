-- Migration: 022_fix_pace_format_monitoring_karo.sql
-- Description: Format pace columns to M'SS"/km in view_athlete_monitoring_karo
...
SELECT 
    ...
    CASE 
        WHEN COALESCE(a.manual_interval_pace_mean, a.interval_pace_mean) IS NOT NULL 
        THEN to_char((COALESCE(a.manual_interval_pace_mean, a.interval_pace_mean) * '1 minute'::interval), 'FMMI''SS"/km')
        ELSE NULL
    END AS "Allure Moy",
    ...
    CASE 
        WHEN COALESCE(a.manual_interval_pace_last, a.interval_pace_last) IS NOT NULL 
        THEN to_char((COALESCE(a.manual_interval_pace_last, a.interval_pace_last) * '1 minute'::interval), 'FMMI''SS"/km')
        ELSE NULL
    END AS "Allure (Last)",
