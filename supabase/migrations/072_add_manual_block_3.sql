-- Add manual interval block 3 columns to activities table
ALTER TABLE activities
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_power_mean double precision,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_power_last double precision,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_hr_mean double precision,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_hr_last double precision,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_pace_mean double precision,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_pace_last double precision,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_count integer,
  ADD COLUMN IF NOT EXISTS manual_interval_block_3_duration_sec double precision;

-- Recreate view_athlete_monitoring_karo with block 3 support
    ...
    SELECT
        ...
        CASE
            WHEN a.manual_interval_power_mean IS NOT NULL
              ...
              OR a.manual_interval_block_3_power_mean IS NOT NULL
              OR a.manual_interval_block_3_power_last IS NOT NULL
              OR a.manual_interval_block_3_hr_mean IS NOT NULL
              OR a.manual_interval_block_3_hr_last IS NOT NULL
              OR a.manual_interval_block_3_pace_mean IS NOT NULL
              OR a.manual_interval_block_3_pace_last IS NOT NULL
            THEN 'Modifié'
            ELSE 'Auto'
        END AS "Source",
        a.session_group_id,
        ...
        -- Block 3
        a.manual_interval_block_3_power_mean,
        a.manual_interval_block_3_power_last,
        a.manual_interval_block_3_hr_mean,
        a.manual_interval_block_3_hr_last,
        a.manual_interval_block_3_pace_mean,
        a.manual_interval_block_3_pace_last,
        ((a.segmented_metrics -> 'interval_blocks') -> 2 ->> 'interval_power_mean')::double precision AS jsonb_block_3_power_mean,
        ((a.segmented_metrics -> 'interval_blocks') -> 2 ->> 'interval_power_last')::double precision AS jsonb_block_3_power_last,
        ((a.segmented_metrics -> 'interval_blocks') -> 2 ->> 'interval_hr_mean')::double precision AS jsonb_block_3_hr_mean,
        ((a.segmented_metrics -> 'interval_blocks') -> 2 ->> 'interval_hr_last')::double precision AS jsonb_block_3_hr_last,
        ((a.segmented_metrics -> 'interval_blocks') -> 2 ->> 'interval_pace_mean')::double precision AS jsonb_block_3_pace_mean,
        ((a.segmented_metrics -> 'interval_blocks') -> 2 ->> 'interval_pace_last')::double precision AS jsonb_block_3_pace_last
    FROM activities a
...
SELECT
    ...
    -- Block 3
    round(COALESCE(manual_interval_block_3_power_mean, jsonb_block_3_power_mean)::numeric, 1) AS interval_block_3_power_mean,
    round(COALESCE(manual_interval_block_3_power_last, jsonb_block_3_power_last)::numeric, 1) AS interval_block_3_power_last,
    round(COALESCE(manual_interval_block_3_hr_mean, jsonb_block_3_hr_mean)::numeric, 1) AS interval_block_3_hr_mean,
    round(COALESCE(manual_interval_block_3_hr_last, jsonb_block_3_hr_last)::numeric, 1) AS interval_block_3_hr_last,
    round(COALESCE(manual_interval_block_3_pace_mean, jsonb_block_3_pace_mean)::numeric, 2) AS interval_block_3_pace_mean,
    round(COALESCE(manual_interval_block_3_pace_last, jsonb_block_3_pace_last)::numeric, 2) AS interval_block_3_pace_last,
    CASE
        WHEN COALESCE(manual_interval_block_3_pace_mean, jsonb_block_3_pace_mean) IS NULL THEN NULL
        ELSE to_char(COALESCE(manual_interval_block_3_pace_mean, jsonb_block_3_pace_mean) * '00:01:00'::interval, 'FMMI''SS"/km"')
    END AS interval_block_3_pace_mean_fmt,
    CASE
        WHEN COALESCE(manual_interval_block_3_pace_last, jsonb_block_3_pace_last) IS NULL THEN NULL
        ELSE to_char(COALESCE(manual_interval_block_3_pace_last, jsonb_block_3_pace_last) * '00:01:00'::interval, 'FMMI''SS"/km"')
    END AS interval_block_3_pace_last_fmt,
    session_group_type,
    session_group_id,
    session_group_role,
    session_group_order,
