-- Migration: 032_add_formatted_block_pace_columns.sql
-- Description: Add pre-formatted block pace columns (mm'ss/km) to monitoring view for Retool.
-- Date: 2026-02-12
...
SELECT 
    ...
    CASE
        WHEN (segmented_metrics->'interval_blocks'->0->>'interval_pace_mean') IS NULL THEN NULL
        ELSE to_char((((segmented_metrics->'interval_blocks'->0->>'interval_pace_mean')::float) * '1 minute'::interval), 'FMMI''SS"/km')
    END AS interval_block_1_pace_mean_fmt,
    CASE
        WHEN (segmented_metrics->'interval_blocks'->0->>'interval_pace_last') IS NULL THEN NULL
        ELSE to_char((((segmented_metrics->'interval_blocks'->0->>'interval_pace_last')::float) * '1 minute'::interval), 'FMMI''SS"/km')
    ...
    CASE
        WHEN (segmented_metrics->'interval_blocks'->1->>'interval_pace_mean') IS NULL THEN NULL
        ELSE to_char((((segmented_metrics->'interval_blocks'->1->>'interval_pace_mean')::float) * '1 minute'::interval), 'FMMI''SS"/km')
    END AS interval_block_2_pace_mean_fmt,
    CASE
        WHEN (segmented_metrics->'interval_blocks'->1->>'interval_pace_last') IS NULL THEN NULL
        ELSE to_char((((segmented_metrics->'interval_blocks'->1->>'interval_pace_last')::float) * '1 minute'::interval), 'FMMI''SS"/km')
